...
`
  ...
  var init = async () => {
    ...
    try {
      const hints = await sendMessage("dashboard", {
    ...
    try {
      const tokens = await sendMessage("dashboard", {
    ...
    try {
      const details = await sendMessage("getAssetContent", {
        url: assetKey
      });
      cmEditor.setOption("trustedSource", details.trustedSource === true);
      cmEditor.setValue(details && details.content || "");
      if (details.sourceURL) {
        const sourceUrlElem = document.querySelector(".cm-search-widget .sourceURL");
        if (sourceUrlElem) {
