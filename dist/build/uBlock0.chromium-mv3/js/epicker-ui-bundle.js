...
`
  ...
  // codemirror/ubo-static-filtering.js
  ...
  var uBOStaticFilteringMode = /* @__PURE__ */ (() => {
  ...
  function initHints() {
    ...
    const getNetOptionHints = function(cursor, seedLeft, seedRight) {
      ...
      const hints = [];
      for (let [text, desc] of netOptionTokenDescriptors) {
  ...
  })());
  {
    ...
    const processInsertion = (doc, deadline, change) => {
      let { from, to: to2 } = change;
      doc.eachLine(from, to2, (lineHandle) => {
    ...
    const onGutterClick = (cm, lineNo, gutterId, ev) => {
      ...
      if (marker.dataset.fold === "end") {
        let depth = 1;
        let lineNo2 = lineHandle.lineNo();
        while (lineNo2--) {
          const prevLineHandle = doc.getLineHandle(lineNo2);
  ...
  }
  {
    ...
    CodeMirror.defineInitHook((cm) => {
      cm.setOption("configureMouse", function(cm2, repeat) {
        return {
  ...
  var debugLog = function(source) {
    ...
    DEBUG_LOGS.push((/* @__PURE__ */ new Date()).toISOString() + " " + msg);
    console.log("[EPICKER-UI]", msg);
  };
  self.getDebugLogs = function() {
    return DEBUG_LOGS.join("\n");
  };
  ...
  (() => {
    ...
    const staticFilteringParser = new AstFilterParser({
      interactive: true,
      nativeCssHas: vAPI.webextFlavor.env.includes("native_css_has")
    });
    ...
    const onPickerMessage = function(msg) {
      ...
      switch (msg.what) {
        ...
        case "saveFilterFromZapper": {
          ...
          if (filter !== void 0 && vAPI.messaging && vAPI.messaging.send) {
    ...
    globalThis.addEventListener("message", (ev) => {
      ...
      pickerContentPort.onmessage = (ev2) => {
        const msg2 = ev2.data || {};
        console.log("[EPICKER-UI] \u2605\u2605\u2605 PICKER CONTENT MESSAGE received, msg.what:", msg2.what);
