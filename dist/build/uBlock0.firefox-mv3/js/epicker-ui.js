...
const onPickerMessage = function(msg) {
    ...
    case 'saveFilterFromZapper': {
        ...
        if (filter !== undefined) {
            console.log('[EPICKER-UI] saveFilterFromZapper: sending createUserFilter for:', filter, 'pageURL:', pageURL);
            vAPI.messaging.send('elementPicker', {
                what: 'createUserFilter',
                autoComment: true,
                filters: filter,
                ...
                console.log('[EPICKER-UI] saveFilterFromZapper: filter saved!');
            }).catch(err => {
                console.error('[EPICKER-UI] saveFilterFromZapper: save failed:', err);
            });
        }
        break;
    }
    case 'dialogCreate': {
        ...
        // In zap mode with stay=true, don't quit - instead re-launch zapper
        if (msg.stay === true && pickerRoot.classList.contains('zap')) {
            debugLog('epicker-ui', 'dialogCreate: in zap mode with stay=true, re-launching zapper');
            quitPicker();
            // Re-launch zapper
            vAPI.messaging.send('elementPicker', {
                ...
                console.error('[EPICKER-UI] dialogCreate: failed to re-launch zapper:', err);
            });
        }
        break;
    }
