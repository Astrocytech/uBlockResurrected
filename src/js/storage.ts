...

{
    ...
    const applyCompiledFilters = (assetKey, compiled) => {
        ...
        console.log('[MV3-STORAGE] After applyCompiledFilters - cosmetic accepted:', cosmeticFilteringEngine.acceptedCount, 'discarded:', cosmeticFilteringEngine.discardedCount);
...
µb.getCompiledFilterList = async function(assetKey) {
    ...
    if (rawDetails.content && rawDetails.content.includes('##')) {
        const cosmeticLines = rawDetails.content.split('\n').filter(line => line.includes('##'));
        console.log('[MV3-STORAGE] Cosmetic filter lines in raw content:', cosmeticLines.length);
        cosmeticLines.slice(0, 5).forEach((line, i) => console.log('[MV3-STORAGE] Line', i, ':', line));
    }
...
µb.compileFilters = function(rawText, details = {}) {
    ...
    while ( lineIter.eot() === false ) {
        ...
        if ( parser.isExtendedFilter() ) {
            ...
            if (line.includes('##')) {
                cosmeticFilterCount++;
                if (cosmeticFilterCount <= 5) {
                    console.log('[MV3-STORAGE] Extended filter (cosmetic):', line);
                }
            }
...
µb.applyCompiledFilters = function(rawText, firstparty) {
    ...
    console.log('[MV3-STORAGE] After applyCompiledFilters - cosmetic accepted:', cosmeticFilteringEngine.acceptedCount, 'specificFilters size:', cosmeticFilteringEngine.specificFilters.size);
};
        ...
        try {
            const results = await Promise.all([
                ...
                io.fromCache('selfie/staticExtFilteringEngine').then(selfie => {
                    ...
                    console.log('[MV3-STORAGE] After fromSelfie - cosmetic accepted:', cosmeticFilteringEngine.acceptedCount, 'specificFilters size:', cosmeticFilteringEngine.specificFilters ? cosmeticFilteringEngine.specificFilters.size : 'N/A');
                    return result;
                }),
            ...
            if ( results.every(v => v) ) {
                console.log('[MV3-STORAGE] Selfie loaded successfully, cosmetic accepted:', cosmeticFilteringEngine.acceptedCount);
