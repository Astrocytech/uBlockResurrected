...
const onMessage = function(request, sender, callback) {
    ...
    case 'reloadAllFilters':
        ...
        µb.loadFilterLists().then(( ) => { 
            ...
            // Log cosmetic filter stats after reload
            if (typeof cosmeticFilteringEngine !== 'undefined') {
                console.log('[MV3-DEBUG] Cosmetic filters - accepted:', cosmeticFilteringEngine.acceptedCount, 'discarded:', cosmeticFilteringEngine.discardedCount);
                console.log('[MV3-DEBUG] Specific filters count:', cosmeticFilteringEngine.specificFilters ? 'exists' : 'missing');
            }
            callback(); 
        });
        return;
    ...
    case 'createUserFilter':
        ...
        // Also store in Blocker Core for future integration
        try {
            const url = new URL(request.docURL);
            const site = url.hostname;
            const isCosmetic = request.filters && request.filters.includes('##');
            console.log('[MSG] createUserFilter - site:', site, 'isCosmetic:', isCosmetic);
            if (isCosmetic) {
                const selectors = request.filters.split('\n').filter(f => f.includes('##'));
                console.log('[MSG] createUserFilter - selectors:', selectors);
                for (const selector of selectors) {
                    const cssSelector = selector.replace(/^.*##/, '');
                    if (cssSelector) {
                        console.log('[MSG] createUserFilter - adding to blockerAdapter:', site, cssSelector);
                        blockerAdapter.addCosmeticSelector(site, cssSelector, true).catch(() => {});
                    }
                }
            }
        } catch (e) {
            console.error('[MSG] createUserFilter error:', e);
        }
        return µb.createUserFilters(request).then(() => {
...
const onMessage = function(request, sender, callback) {
    ...
    case 'writeUserFilters':
        ...
        console.log('[MV3-DEBUG] Content preview:', request.content ? request.content.substring(0, 500) : 'empty');
        console.log('[MV3-DEBUG] Enabled:', request.enabled, 'Trusted:', request.trusted);
        if ( request.content ) {
            const cosmeticLines = request.content.split('\n').filter(line => line.includes('##'));
