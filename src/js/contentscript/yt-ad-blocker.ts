    ...
    const stripAdDataFromObject = (obj, depth = 0) => {
        ...
        for (const key of Object.keys(obj)) {
            ...
            const isAdKey = AD_KEYS.includes(key) || 
                           ...
                           key === 'trackingParams' && typeof obj[key] === 'string' && obj[key].startsWith('AB');
        ...
        JSON.parse = function(text, reviver) {
            try {
                ...
                if (isYouTube() && typeof parsed === 'object' && parsed !== null) {
                    const jsonStr = text;
                    if (jsonStr.includes('"adPlacements"') || 
                        jsonStr.includes('"playerAds"') ||
                        jsonStr.includes('"adSlots"')) {
            ...
            for (const selector of scriptSelectors) {
                ...
                for (const script of scripts) {
                    try {
                        ...
                        if (text && text.length > 0 && (text.includes('"adPlacements"') || text.includes('"playerAds"'))) {
                            const stripped = parseAndStripResponse(text);
                            if (stripped !== text) {
