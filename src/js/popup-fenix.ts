...
Promise.resolve(vAPI.localStorage?.getItemAsync?.("popupFontSize"))
  .then((value) => {
...
Promise.resolve(vAPI.localStorage?.getItemAsync?.("popupPanelSections"))
  .then((bits) => {
...
const reCyrillicNonAmbiguous =
  /[\u0400-\u042b\u042d-\u042f\u0431\u0432\u0434\u0436-\u043d\u0442\u0444\u0446-\u0449\u044b-\u0454\u0457\u0459-\u0460\u0462-\u0474\u0476-\u04ba\u04bc\u04be-\u04ce\u04d0-\u0500\u0502-\u051a\u051c\u051e-\u052f]/;
const reCyrillicAmbiguous =
  /[\u042c\u0430\u0433\u0435\u043e\u043f\u0440\u0441\u0443\u0445\u044a\u0455\u0456\u0458\u0461\u0475\u04bb\u04bd\u04cf\u0501\u051b\u051d]/;
...
const updateAllFirewallCells = function (doRules = true, doCounts = true) {
  ...
  if (popupData.firewallRules === undefined) {
    return;
  }
  if (popupData.hostnameDict === undefined) {
    return;
  }
...
const expandHostnameStats = () => {
  for (const des of allHostnameRows) {
    const hnDetails = popupData.hostnameDict[des];
    if (hnDetails === undefined || hnDetails.counts === undefined) {
      continue;
    }
    ...
    if (dnDetails === undefined || dnDetails.counts === undefined) {
      continue;
    }
    if (isDomain) {
...
dom.on("#firewall .filterExpressions", "click", "span[data-expr]", (ev) => {
  ...
  filterFirewallRows();
  const elems = qsa$("#firewall .filterExpressions span[data-expr]");
  const filters = Array.from(elems).map((el) =>
    dom.cl.has(el, "on") ? "1" : "0",
  );
  filters.unshift("00");
...
const renderPrivacyExposure = function () {
  ...
  const keys = hostnameDict
    ? Object.keys(hostnameDict).sort(hostnameCompare)
    : [];
...
const renderPopup = function () {
  ...
  // The hostname information below the power switch
  {
    ...
    if (pageDomain !== "") {
      ...
      dom.text(
        elemHn,
        pageHostname !== pageDomain
          ? safePunycodeToUnicode(
...
const renderTooltips = function (selector) {
  for (const [key, details] of tooltipTargetSelectors) {
    if (selector !== undefined && key !== selector) {
      continue;
    }
    const elem = qs$(key);
