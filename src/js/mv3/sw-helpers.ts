...
export const getTabForRequest = async (tabId?: number | null): Promise<chrome.tabs.Tab | undefined> => {
...
export const pickMostRelevantBrowsingTab = async (): Promise<chrome.tabs.Tab | undefined> => {
    const [tab] = await chrome.tabs.query({ active: true, currentWindow: true });
...
export const updateToolbarIcon = async (tabId: number, options: { filtering?: boolean; clickToLoad?: string }) => {
    try {
        ...
        if (isFilteringEnabled && (currentParts & 0b001)) {
            const getMatchedBlockedRequestCount = async (tabId: number, minTimeStamp = 0) => {
...
export const extractListURLs = (text: string): string[] => text
    .split(/\s+/)
    .map(line => line.trim())
...
export const cloneObject = <T>(value: T): T => JSON.parse(JSON.stringify(value));

export const deriveDefaultSelectedFilterLists = (available: Record<string, any>, userPath: string): string[] => {
...
export const resolveBundledFilterListPath = (asset: any): string | undefined => {
...
export const hostnameMatchesFilterScope = (pageHostname: string, scope: string): boolean => {
...
export const buildSpecificCosmeticPayload = (
    ...
    for ( const entry of storedData.specificCosmeticFilters ) {
        ...
        const included = matches.length === 0
            ? true
            : matches.some((scope: string) => hostnameMatchesFilterScope(pageHostname, scope));
        if ( included === false ) { continue; }
        const excluded = excludeMatches.some((scope: string) => hostnameMatchesFilterScope(pageHostname, scope));
...
export const pickerContextPoints = new Map<string, {
    x: number;
    y: number;
    timestamp: number;
}>();

export const pickerContextPointKey = (tabId: number, frameId: number) => `${tabId}:${frameId}`;
