...
export const compilePowerSwitchDnrRules = async (
  ...
  // If all or most domains are disabled, we're in global OFF mode
  // In this case, don't add any rules - just let traffic pass through normally
  const entries = Object.entries(perSiteFiltering);
  const falseCount = entries.filter(([, v]) => v === false).length;

  // If more than 50% of entries are false, treat as global OFF and don't add rules
  // This prevents adding thousands of rules when filtering is disabled
...
export const syncPowerSwitchDnrRules = async (): Promise<void> => {
  ...
  const toRemove = existing
    .map((r) => r.id)
    .filter((id) => id >= POWER_RULE_ID_MIN && id < 9300000);
