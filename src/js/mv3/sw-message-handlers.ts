...
export type MessageHandlersDeps = {
    ...
    getFirewallRulesForPopup: (srcHostname: string, hostnameDict: Record<string, any>) => Record<string, string>;
    ...
    const handleDashboardMessage = async (request: PopupRequest) => {
        ...
        case 'applyFilterListSelection':
            return applyFilterListSelection(request as {
                ...
                toImport?: string;
                toRemove?: string[];
            }, popupState, () => Promise.resolve());
        case 'reloadAllFilters':
            return reloadAllFilterLists(popupState, () => Promise.resolve());
        case 'updateNow':
            return updateFilterListsNow(undefined, popupState, () => Promise.resolve());
        case 'listsUpdateNow':
            return updateFilterListsNow(request as { assetKeys?: string[]; preferOrigin?: boolean }, popupState, () => Promise.resolve());
        case 'userSettings':
            return setUserSetting(request);
        case 'getLocalData':
            return getLocalData();
        ...
        case 'setWhitelist': {
            ...
            if (Array.isArray(whitelist)) {
                const whitelistText = µb?.whitelistToString?.(whitelist) || whitelist.join('\n');
        ...
        case 'getFirewallRules': {
            ...
            const desHostnames = request.desHostnames as Record<string, unknown> || {};
            const hostnameDict = request.hostnameDict as Record<string, any> || {};
