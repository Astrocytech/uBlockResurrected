...
export const createToggleHandlers = (deps: ToggleHandlersDeps) => {
  ...
  const toggleNetFiltering = async (request: PopupRequest) => {
    ...
    try {
      ...
      if (typeof tabId === "number") {
        console.log(
          "[MV3] toggleNetFiltering: calling updateToolbarIcon with state =",
          state,
        );
...
export const toggleHostnameSwitch = createToggleHandlers(
  {} as ToggleHandlersDeps,
).toggleHostnameSwitch;
export const toggleNetFiltering = createToggleHandlers(
  {} as ToggleHandlersDeps,
).toggleNetFiltering;
export const toggleFirewallRule = createToggleHandlers(
  {} as ToggleHandlersDeps,
).toggleFirewallRule;
export const saveFirewallRules = createToggleHandlers(
  {} as ToggleHandlersDeps,
).saveFirewallRules;
export const revertFirewallRules = createToggleHandlers(
  {} as ToggleHandlersDeps,
).revertFirewallRules;
