...
test('popup loads and shows switch', async () => {
    const server = createServer((_, r) => { r.writeHead(200); r.end('<h1>Test</h1>'); });
    server.listen(0);
    ...
    await popup.goto(`chrome-extension://${extId}/popup-fenix.html`, { waitUntil: 'domcontentloaded' });
    ...
    const cls = await popup.locator('body').getAttribute('class');
    ...
    expect(dynamicRules.some(rule => rule.action?.type === 'allowAllRequests')).toBe(true);
